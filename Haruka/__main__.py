...
def start(bot: Bot, update: Update, args: List[str]):
    ...
    if update.effective_chat.type == "private":
        ...
        update.effective_message.reply_text("I'm onlineâï¸")
...
def send_start(bot, update):
    ...
    keyboard = [[InlineKeyboardButton(text="ð®ð³ Language", callback_data="set_lang")]]
    keyboard += [[InlineKeyboardButton(text="ð  Reporting", callback_data="cntrl_panel_M"), 
...
def control_panel(bot, update):
    ...
    if M_match:
        text = "*Control panel* ð "

        keyboard = [[InlineKeyboardButton(text="ð¤ My settings", callback_data="cntrl_panel_U(1)")]]
        ...
        if conn:
            ...
            if member.status in ('administrator', 'creator'):
                ...
                keyboard += [[InlineKeyboardButton(text="ð¥ Group settings", callback_data="cntrl_panel_G_back")]]
            elif user.id in SUDO_USERS:
                text += f"\nConnected chat - *{chatG.title}* (you sudo)"
                keyboard += [[InlineKeyboardButton(text="ð¥ Group settings (SUDO)", callback_data="cntrl_panel_G_back")]]
            else:
